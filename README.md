...
- 11 张卡的标题在小尺寸卡面上会**偏高约半行**。这些卡的预制体在标题处摆的不是一个
  文本对象，而是位置固定的 `Title1` / `Title2` 两个（英文排两行）。中文短，用不着换行，
  于是把整个标题塞进 `Title1`、`Title2` 填一个空格，就只占了上面那个槽位。
  要真正居中得改预制体里的 RectTransform 坐标，权衡后没做。
  中文超过 7 字的 `Card_016` / `Card_068` / `Card_110` 仍保持两行拆分。
  （`Title2` 必须填一个空格，不能留空串——空串实测会出问题，
  原始数据里也没有任何空值先例。）
